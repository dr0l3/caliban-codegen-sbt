//package apistuff
//
//import apistuff.EntityResolvers.createOrderEntityResolver
//import apistuff.Util.{getValueAtKey, isKeySet}
...
//import codegen.Util.{argToWhereClause, createExtensionModifications, extendType, extractTypeName, toIntermediate, toIntermediateFromReprQuery, toQueryWithJson}
//import codegen.{JsonPathPart, ObjectExtension, ObjectExtensionField, Table}
//import generic.Util.ExtensionPosition
...
//  }
//
//  def genericQueryHandler[R](tables: List[Table], extensions: List[ExtensionPosition], connection: Connection)(field: Field): Step[R] = {
//    // We have created an intermediate that has filtered out all the fields that have @requires
//    val intermediate = toIntermediate(field, tables, "root", extensions)
//    val fieldMask: List[Json => Json] = Nil // Transformations for removing the fields that were added to satisfy extensions
//
...
//
//
//  def genericEntityHandler[R](extensions: Map[ExtensionPosition, (Field,Json) => ZIO[Any, Throwable, JsonObject]], tables: List[Table], conditionsHandlers: Map[String,ObjectValue => String], connection: Connection)(value: InputValue): ZQuery[R, CalibanError, Step[R]] = {
...
//
//
//  def constructQueryOperation[R, S](connection: Connection, extensions: List[ExtensionPosition], tables: List[Table])(implicit schema: Schema[R,S]): Operation[R] = {
